...
public class Interface {
	...
	public void gerarMenu(){
		...
		while(true){
			try {
				...
				JOptionPane.showMessageDialog(null, "Você deve utilizar somente numeros inteiros.");
			}catch (Exception e) {
	...
	private void menuAction(String action) throws Exception, NumberFormatException{
	...
	private void cadastrarNovaPublicacao() throws NumberFormatException, Exception{
		switch (Integer.valueOf(JOptionPane.showInputDialog("1 - Livro\n2 - Periodico\ntipo:"))) {
		case 1://Cadastrar um novo usuario
			cadastrarNovoLivro();
			break;
	...
	private void cadastrarNovoLivro() throws NumberFormatException{
		int codPublicacao = Integer.valueOf(JOptionPane.showInputDialog("Codigo da Publicacao"));
		...
		int ano = Integer.valueOf(JOptionPane.showInputDialog("Ano da Publicacao"));
		String autores = JOptionPane.showInputDialog("Autores da Publicacao");
		int qtdeExamplares = Integer.valueOf(JOptionPane.showInputDialog("Quantidade de Examplares da Publicacao"));
		
		Publicacao publicacao = new Livro(codPublicacao, titulo, editora, ano, autores, qtdeExamplares);
	...
	private void cadastrarNovoPeriodico() throws NumberFormatException{
		...
		String editora = JOptionPane.showInputDialog("Editora da Publicacao");
		int ano = Integer.valueOf(JOptionPane.showInputDialog("Ano da Publicacao"));
		int numEdicao = Integer.valueOf(JOptionPane.showInputDialog("Autores da Publicacao"));
		int mes = Integer.valueOf(JOptionPane.showInputDialog("Quantidade de Examplares da Publicacao"));
		
		Publicacao publicacao = new Periodico(codPublicacao, titulo, editora, ano, numEdicao, mes);
		biblioteca.getPublicacoes().add(publicacao);
	}
	
	
	private void cadastrarNovoEmprestimo() throws NumberFormatException, Exception{
		String cpf = JOptionPane.showInputDialog("Cpf do usuario:");
		Usuario usuario = new Usuario("", cpf, "", "");
		int indexusuario = biblioteca.getUsuarios().indexOf(usuario);

		Calendar dataPrevDevolucao = GregorianCalendar.getInstance();
		dataPrevDevolucao.add(GregorianCalendar.DAY_OF_MONTH, 7);
	
		Emprestimo emprestimo = new Emprestimo(biblioteca.getUsuarios().get(indexusuario), dataPrevDevolucao);
	...
	private void cadastrarItemParaEmprestimo(Emprestimo emprestimo) throws NumberFormatException, Exception{
		...
		if(publicacao != null){
			...
			if(publicacao instanceof Emprestavel)	
				...
				JOptionPane.showMessageDialog(null, "Essa publicação não pode ser emprestada");
		}else{
			JOptionPane.showMessageDialog(null, "Não existe esse elemento");
		}
		
		if(JOptionPane.showConfirmDialog(null, "Deseja inserir mais uma publicacao") == 0)
			cadastrarItemParaEmprestimo(emprestimo);
	}
	
	private void cadastrarItemParaEmprestimo() throws NumberFormatException, Exception{	
	...
	private Emprestimo buscarEmprestimo(){
		int numeroEmprestimo = Integer.valueOf(JOptionPane.showInputDialog("Numero do Emprestimo"));
	...
	private void excluirItemEmprestimo() throws NumberFormatException, Exception{
		Emprestimo emprestimo = buscarEmprestimo();
		Publicacao publicacao = biblioteca.getPublicacao(getCodPublicacao(emprestimo));
		biblioteca.remove(emprestimo, (Emprestavel)publicacao);
	}
	
	private int getCodPublicacao(Emprestimo emprestimo){
		return Integer.valueOf(JOptionPane.showInputDialog(emprestimo.toString() + "\n" + "Item a ser deletado:"));
	}
